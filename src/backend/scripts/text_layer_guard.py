...
背景：部分客户材料是「先被别的软件 OCR 过一遍」的可搜索 PDF。Docling 默认信任
已有文本层、不再自己识别，于是那一份烂文本被原样送进模型。实测新鸿审计报告
2023 的合并资产负债表页，文本层里的金额全是 `767、 7(Xl^00000` 这种形态，
强制整页 OCR 后同页数字全部可还原。

判据只数「确凿损坏的金额」，不算规范率。

2026-07-31 教训：初版用「金额候选里规范的占比 <50%」判定，把个人征信报告判成
不可信——那页的所谓「畸形金额」全是列表序号 `1.` `2.` … `9.`，文本层其实完好。
误判方向是最坏的：把一份准确的文本层扔掉换成 OCR 结果。比例判据对「什么算候选」
极其敏感，改为直接数损坏证据后，序号和日期再多也不会触发。
"""
